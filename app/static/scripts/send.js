...
document.addEventListener('DOMContentLoaded', function (event) {
    ...
    function sendPackage(form){
        ...
        let failureMessage = "Generowanie listu przewozowego nie powiodło się. Prawdopodobnie wygasła ważność twojego tokenu.";
        ...
        fetch(url, sendParams).then(response => {
            console.log("odpowiedź: " + response.message)
    ...
    function getResponseData(response) {
        ...
        if (response.status == HTTP_STATUS.CREATED || response.status == HTTP_STATUS.OK) {
            ...
            let id = "button-submit-form";
            console.log('nadanie wiadomości')
            sendMessage(ADD_PACK_ROOM, 'paczka została dodana.')

...
const ADD_PACK_ROOM = "add_pack_room"
...
function loadWaybills(page_url){
    ...
    fetchPacks(page_url).then(response => {
            ...
            if (n_of_waybills > 0){
                let waybills_count = response.n_of_all_waybills
                let waybills = response.waybills
                let images = response.waybills_images
                let dates = response.waybills_dates
                ...
                waybills.forEach((waybill, idx) =>
                    addWaybillToList(tbody, waybill, images[idx], pack_states[idx], dates[idx])
                )
                table.appendChild(tbody)
                console.log('stworzenie tabeli')
                clearTable()
                
                let n_waybill = document.createElement('div')
                n_waybill.className = "max-width-elem empty"
                n_waybill.innerText = 'Liczba twoich paczek: ' + waybills_count 
                h1.insertAdjacentElement('afterend', n_waybill)
                n_waybill.insertAdjacentElement('afterend', table)
...
function addWaybillToList(tbody, waybill, image, state, date){
    let row = tbody.insertRow()
    ...
    cell = row.insertCell()
    let creation_date = document.createTextNode(date)
    cell.appendChild(creation_date)

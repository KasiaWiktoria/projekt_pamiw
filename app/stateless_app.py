                ...
                try:
                    ...
                    if os.path.exists(fullname):
                        ...
                        if os.path.exists(img_path):
                            ...
                            return {'message' : 'Nie można usunąć obrazu.', 'status':400}, 400
                    else:
                        return {'message' : 'Nie można usunąć pliku pdf.', 'status':400}, 400
                    try:
                        if db.hexists(user + '-'+ PACKNAMES, fullname) and db.hexists(pack_id, PATH_AND_FILENAME) and db.hexists(pack_id, 'status') and db.hexists(IMAGES_PATHS, pack_id):
                            ...
                            return {'message' : 'Czegoś nie ma w bazie danych.', 'status':404}, 404
                        
                    except:
                        log.debug('Nie można usunąć plików z bazy danych.')
                        return {'message' : 'Nie można usunąć plików z bazy danych.', 'status':400}, 400
                    
                except:
                    log.debug('Nie można usunąć plików.')
                    return {'message' : 'Nie można usunąć plików.', 'status':400}, 400
                log.debug('usunięto')
                return {'message' : 'Poprawnie usunięto paczkę.', 'status':200}, 200
            else:
                return { 'message': 'Nie można usunąć paczki, której status został już zmieniony.', 'status':400}, 400
        else:
            return {'message' : 'Nie ma takiej paczki w bazie danych.', 'status':404}, 404
...
class WaybillCreator(Resource):
    ...
    def post(self):
        try:
            ...
            try:
                ...
                log.debug('Token wygasł.')
                response = make_response({'message': "Token wygasł.", 'status':401}, 401)
                return response
    ...
    def save_waybill(self, user, waybill):
        ...
        db.hset(pack_id, 'creation_date', str_date)
        db.hset(IMAGES_PATHS, pack_id , waybill.get_img_path())
        db.hset(user + '-'+ PACKNAMES, fullname, pack_id)

        log.debug(f'dodano paczkę {pack_id} dla użytkownika {user}')
